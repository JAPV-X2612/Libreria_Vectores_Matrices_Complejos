...
class Test_Libreria_Nombre(ut.TestCase):
    ...
    def test_cplx_vct_mtx_adj(self):
        ...
        vma = [list(m) for m in cplx_vct_mtx_adj([[-4.6+3j, 1.1-1j], [21+7j,-1+2j]])]
    ...
    def test_cplx_mtx_prod(self):
        ...
        None -> OK or FAILED (failures=#)"""
        mr = [list(m) for m in cplx_mtx_prod([[1+3j, -5+1j], [4-1j, 8+4j]], [[1-1j, 2-6j], [3+9j, -4+2j]])]
        self.assertEqual(mr, [[-20-40j, 38-14j], [-9+79j, -38-26j]])
        mr = [list(m) for m in cplx_mtx_prod([[-1+2j, 6-7j], [1-8j, 5]], [[1j, 1-1j], [1+7j, 0]])]
